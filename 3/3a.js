...
function moveAndGetNewLine(actionString, lastPoint) {
  ...
  switch (actionName) {
    case "R":
      return [{ ...lastPoint }, { x: lastPoint.x + distance, y: lastPoint.y }];
    case "L":
      return [{ ...lastPoint }, { x: lastPoint.x - distance, y: lastPoint.y }];
    case "U":
      return [{ ...lastPoint }, { x: lastPoint.x, y: lastPoint.y + distance }];
    case "D":
      return [{ ...lastPoint }, { x: lastPoint.x, y: lastPoint.y - distance }];
    default:
...
let line1 = moveAndGetNewLine(path1[0], { x: 0, y: 0 });
let line2 = moveAndGetNewLine(path2[0], { x: 0, y: 0 });
...
for (let i = 1; i <= path1.length; i++) {
  for (let k = 1; k <= path2.length; k++) {
    if (k === 1 && i !== 1) {
      // reset line 2
      line2 = moveAndGetNewLine(path2[0], { x: 0, y: 0 });
    }
...
const distance = intersectionsArray
  .map(lines => {
    const point = intersectionPoint(lines.l1, lines.l2);
    return point ? Math.abs(point.x) + Math.abs(point.y) : null;
  })
  .filter(d => !!d)
