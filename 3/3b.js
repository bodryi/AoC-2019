...
// const data1 = "R8,U5,L5,D3";
// const data2 = "U7,R6,D4,L4";
const path1 = data1.split(",");
const path2 = data2.split(",");
// Lengths are equal

function checkY(l1, l2) {
  return (
    (l2[0].y <= l1[0].y && l2[0].y >= l1[1].y) ||
    (l2[0].y >= l1[0].y && l2[0].y <= l1[1].y) ||
    (l1[0].y <= l2[0].y && l1[0].y >= l2[1].y) ||
    (l1[0].y >= l2[0].y && l1[0].y <= l2[1].y)
  );
}

function checkX(l1, l2) {
  return (
    (l2[0].x <= l1[0].x && l2[0].x >= l1[1].x) ||
    (l2[0].x >= l1[0].x && l2[0].x <= l1[1].x) ||
    (l1[0].x <= l2[0].x && l1[0].x >= l2[1].x) ||
    (l1[0].x >= l2[0].x && l1[0].x <= l2[1].x)
  );
}

function intersects(l1, l2) {
  return checkX(l1, l2) && checkY(l1, l2);
}

function moveAndGetNewLine(actionString, lastPoint) {
  const actionName = actionString[0];
  const distance = +actionString.substring(1);
  switch (actionName) {
    case "R":
      return [
        { ...lastPoint },
        {
          ...
          y: lastPoint.y,
          steps: lastPoint.steps + distance,
          op: actionString
        }
      ];
    case "L":
...
function intersectionPoint(l1, l2) {
  ...
  if (testLine1[0].x >= testLine1[1].x) {
    testLine1[0] = { ...l1[1] };
    testLine1[1] = { ...l1[0] };
  }
  if (testLine2[0].x >= testLine2[1].x) {
    testLine2[0] = { ...l2[1] };
    testLine2[1] = { ...l2[0] };
  }

  const p1 = testLine1[0];
  const p2 = testLine1[1];
  const p3 = testLine2[0];
  const p4 = testLine2[1];
  ...
  if (ta >= 0 && ta <= 1 && tb >= 0 && tb <= 1) {
    const dx = p1.x + ta * (p2.x - p1.x);
    const dy = p1.y + ta * (p2.y - p1.y);

    const point = { x: dx, y: dy };

    const lastSteps1 = stepsUntilIntersection(l1, point);
    const lastSteps2 = stepsUntilIntersection(l2, point);

    return {
      x: dx,
      y: dy,
      steps: l1[0].steps + l2[0].steps + lastSteps1 + lastSteps2
    };
  }
...
function stepsUntilIntersection(line, intersectionPoint) {
  try {
    const actionName = line[1].op[0];
    switch (actionName) {
      case "R":
        return intersectionPoint.x - line[0].x;
      case "L":
        return line[0].x - intersectionPoint.x;
      case "U":
        return intersectionPoint.y - line[0].y;
      case "D":
        return line[0].y - intersectionPoint.y;
      default:
        throw new Error("Unknown action name");
    }
  } catch (e) {
    throw new Error(
      "Can't calculate steps until intersection, check stepsUntilIntersection method"
    );
  }
}

let line1 = moveAndGetNewLine(path1[0], { x: 0, y: 0, steps: 0, op: null });
let line2 = moveAndGetNewLine(path2[0], { x: 0, y: 0, steps: 0, op: null });
...
for (let i = 1; i <= path1.length; i++) {
  for (let k = 1; k <= path2.length; k++) {
    if (k === 1 && i !== 1) {
      // reset line 2
      line2 = moveAndGetNewLine(path2[0], {
        x: 0,
        y: 0,
        steps: 0,
        op: null
    ...
    if (intersects(line1, line2)) {
      intersectionsArray.push({ l1: [...line1], l2: [...line2] });
    }
    if (k === path2.length) {
      break;
    }
    line2 = moveAndGetNewLine(path2[k], line2[1]);
  }
  if (i === path1.length) {
    break;
  }
  line1 = moveAndGetNewLine(path1[i], line1[1]);
}

const minSteps = intersectionsArray
  .map(lines => {
    const point = intersectionPoint(lines.l1, lines.l2);
    return point ? point.steps : null;
  })
  .filter(s => !!s)
