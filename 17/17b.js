...
function getSurroundingScaffolds(map, pos) {
  const left = map.find(el => el.y === pos.y && el.x === pos.x - 1);
  const right = map.find(el => el.y === pos.y && el.x === pos.x + 1);
  const top = map.find(el => el.y === pos.y - 1 && el.x === pos.x);
  const bottom = map.find(el => el.y === pos.y + 1 && el.x === pos.x);
  let res = {
    LEFT: left && left.value !== blocks.OPEN_SPACE ? left : null,
    RIGHT: right && right.value !== blocks.OPEN_SPACE ? right : null,
    UP: top && top.value !== blocks.OPEN_SPACE ? top : null,
    DOWN: bottom && bottom.value !== blocks.OPEN_SPACE ? bottom : null,
    count: 0,
    availableDirections: []
  };
  for (let key in res) {
    if (res[key] && key in directions) {
      res.availableDirections.push(key);
      res.count++;
    }
  }
  return res;
  ...
  switch (currentDirection) {
    case directions.UP: {
      switch (neededDirection) {
        ...
        default:
          throw new Error(
            `Need more than 1 step to turn around from ${currentDirection} to ${neededDirection}`
          );
      }
    }
    case directions.DOWN: {
    ...
    default:
      throw new Error(`Unknown direction: ${currentDirection}`);
  }
}
