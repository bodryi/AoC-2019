...
const minX = res.plates.reduce(
  (minX, curr) => (curr.coords.x < minX ? curr.coords.x : minX),
  0
);
const maxX = res.plates.reduce(
  (maxX, curr) => (curr.coords.x > maxX ? curr.coords.x : maxX),
  0
);
const minY = res.plates.reduce(
  (minY, curr) => (curr.coords.y < minY ? curr.coords.y : minY),
  0
);
const maxY = res.plates.reduce(
  (maxY, curr) => (curr.coords.y > maxY ? curr.coords.y : maxY),
  0
);
...
for (let i = Math.abs(minY) + Math.abs(maxY); i >= 0; i--) {
  for (let j = 0; j <= Math.abs(minX) + Math.abs(maxX); j++) {
    const coloredPlate = res.plates.find(
      plate => plate.coords.x === j + minX && plate.coords.y === i + minY
    );

    displayPlate += coloredPlate && coloredPlate.color ? "#" : " ";
  }
  displayPlate += "\n";
}
