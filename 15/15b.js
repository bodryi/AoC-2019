...
function runProgram() {
  ...
  let stack = [labyrinth[0]];
  let currentCell = stack.pop();
  let prevCell = null;
  let { program, relativeBase, currentIndex } = currentCell.programState;
    ...
    switch (operation) {
      ...
      case 4: {
        ...
        if (waveActionIndex % 2) {
          const cell = {
            ...getNewCell(currentCell, waveActionsArray[waveActionIndex - 1]),
            d: currentCell.d + 1,
            type: out,
            path: [...currentCell.path, waveActionsArray[waveActionIndex - 1]],
            programState: {
              program: [...program],
              currentIndex,
              relativeBase
          ...
          if (!labyrinth.find(c => c.x === cell.x && c.y === cell.y)) {
            labyrinth.push(cell);
            if (out !== reply.WALL) {
              stack.push(cell);
            }
        ...
        if (waveActionIndex === waveActionsArray.length) {
          ...
          program = currentCell.programState.program;
          relativeBase = currentCell.programState.relativeBase;
          currentIndex = currentCell.programState.currentIndex;
        }
        break;
      }
      case 5:
