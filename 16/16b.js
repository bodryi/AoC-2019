/**
 *
 * Offset is beyond half of input data, so for every element after offset pattern values will be equal 1
 */

const BOUND = 100;
const REPEAT_TIME = 10000;

function phase(input, patternBase, offset) {
  let value = 0;
  for (let i = input.length - 1; i >= offset; i--) {
    value = (value + input[i]) % 10;
    input[i] = value;
  }
  return input;
}

function getRepeatedInput(input, repeatTimes) {
  const res = new Array(input.length * repeatTimes);
  for (let i = 0; i < repeatTimes; i++) {
    for (let j = 0; j < input.length; j++) {
      res[i * input.length + j] = input[j];
    }
  }
  return res;
}

const input = ""
  .split("")
  .map(n => +n);
const pattern = [0, 1, 0, -1];
const offset = +input.slice(0, 7).join("");

const repeatedInput = getRepeatedInput(input, REPEAT_TIME);
let res = [...repeatedInput];
for (let i = 0; i < BOUND; i++) {
  res = phase([...res], pattern, offset);
}

console.log(res.slice(offset, offset + 8).join(""));
//36265589
